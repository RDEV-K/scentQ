...
@section('content')
    <div class="card mb-3 p-4">
        ...
        <div class="card-body bg-light p-0">
            ...
            <form action="{{ route('staff.collection.update', $collection->id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group text-center">
                            <img class="img-thumbnail w-100"
                                 src="{{ $collection->image??'https://via.placeholder.com/200' }}"
                                 id="logo-preview"
                            >
                            <input type="hidden" name="image" value="{{ $collection->image }}" id="logo-input">
                            <button
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="type" id="type" class="custom-select" required>
                                        <option value="curated" {{ $collection->type == 'curated'?'selected':'' }}>Curated</option>
                                        <option value="capsule" {{ $collection->type == 'capsule'?'selected':'' }}>Capsule</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="for" id="for" class="custom-select" required>
                                        <option value="women" {{ $collection->for == 'women'?'selected':'' }}>Women</option>
                                        <option value="men" {{ $collection->for == 'men'?'selected':'' }}>Men</option>
                                    </select>
                                </div>
                            </div>
                        </div>
