        ...
        <div class="row" id="app" v-cloak>
            <div class="col-md-8">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('sub_title') is-invalid @enderror"
                                           value="{{ old('sub_title') }}" name="sub_title" id="sub_title">
                                    @error('sub_title')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="number" min="0" step="0.01"
                                               ...
                                               value="{{ old('retail_value') }}" name="retail_value" id="retail_value"
                                               required>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="number" min="0" step="0.01"
                                               ...
                                               value="{{ old('additional_price') }}" name="additional_price"
                                               id="additional_price">
                                        <div class="input-group-append"><span
                                                class="input-group-text">{{ config('misc.currency_code') }}</span></div>
                                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control tinymce" name="content" id="content">{{ old('content') }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="how_to_use">@lang('How To Use')</label>
                            <textarea class="form-control tinymce" name="how_to_use" id="how_to_use">{{ old('how_to_use') }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="ingredients">@lang('Ingredients')</label>
                            <textarea class="form-control tinymce" name="ingredients" id="ingredients">{{ old('ingredients') }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="why_we_love_it">@lang('Why We Love It')</label>
                            <textarea class="form-control tinymce" name="why_we_love_it" id="why_we_love_it">{{ old('why_we_love_it') }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="how_it_works">@lang('How It Works')</label>
                            <textarea class="form-control tinymce" name="how_it_works" id="how_it_works">{{ old('how_it_works') }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="disclaimer">@lang('Disclaimer')</label>
                            <textarea class="form-control tinymce" name="disclaimer" id="disclaimer">{{ old('disclaimer') }}</textarea>
                        </div>
                        <div class="form-group row" v-if="images.length">
                            <div v-for="(image, index) in images" class="col-md-4 mb-4 position-relative">
                                <img :src="image.thumb_url" class="img-fluid img-thumbnail border border-dashed border-black">
                                <span class="position-absolute r-0 t-0 bg-danger p-2 text-white rounded" @click.prevent="removeImage(index)">
                                    <i class="fas fa-times"></i>
                                </span>
                                <input type="hidden" :name="'images[' + index + '][type]'" :value="image.type">
                                <input type="hidden" :name="'images[' + index + '][url]'" :value="image.url">
                                <input type="hidden" :name="'images[' + index + '][thumb_url]'" :value="image.thumb_url">
                            </div>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="form-group">
                                <button class="btn btn-outline-primary" @click.prevent="addYoutube">@lang('Add Youtube')</button>
                            </div>
                            <div class="form-group">
                                <button class="btn btn-outline-warning" @click.prevent="changeImages">@lang('Set Gallery Images')</button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="card mb-4" v-if="type !== 'giftset'">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <select class="form-control custom-select custom-select-lg" v-model="tempPurchaseOption">
                                        <option value="subscription" v-if="isSubscriptionPriceCanBeAdded">@lang('Subscription')</option>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <button class="btn btn-outline-primary btn-lg btn-block" @click.prevent="addPurchaseOption">@lang('Add Option')</button>
                                </div>
                            </div>
                        </div>
                        <div class="row p-2" v-for="(purchase_option, index) in purchase_options" :key="index">
                            <div class="col-md-4">
                                <img :src="purchase_option.image??'https://via.placeholder.com/200'" class="img-fluid img-thumbnail">
                                <input type="hidden" :name="'purchase_options[' + index + '][type]'" v-model="purchase_option.type">
                                <input type="hidden" :name="'purchase_options[' + index + '][image]'" v-model="purchase_option.image">
                                <button class="btn btn-outline-warning mt-2" @click.prevent="selectImage(index)">@lang('Set Image')</button>
                            </div>
                            <div class="col-md-8">
                                <div class="row">
                                    <div class="col-md-8">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input type="number" class="form-control" step="0.01" min="0" :id="'price-' + index" :name="'purchase_options[' + index + '][price]'" v-model="purchase_option.price">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" :id="'type_text-' + index" :name="'purchase_options[' + index + '][type_text]'" v-model="purchase_option.type_text">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label :for="'quantity_text-' + index">@lang('Quantity Text')</label>
                                            <input type="text" class="form-control" :id="'quantity_text-' + index" :name="'purchase_options[' + index + '][quantity_text]'" v-model="purchase_option.quantity_text">
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select2 class="form-control form-control-lg" :options="products" name="related_products[]" for="related_products" v-model="related_products" multiple>
                ...
                <div class="row" v-if="computedTaxonomies.length">
                    <div class="col-md-6" v-for="(taxonomy, computedTaxonomyIndex) in computedTaxonomies" :key="computedTaxonomyIndex">
                        <div class="card mb-4">
                            <div class="card-header">
                            ...
                            <div class="card-body">
                                <select2 class="form-control" :name="'terms[' + taxonomy.id + ']'"
                                    ...
                                    <option v-for="(term, termIndex) in taxonomy.terms" :value="term.id">@{{ term.name }}</option>
                                </select2>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="form-group d-flex justify-content-between">
                            <button class="btn btn-success" type="submit" name="status" value="published"
                                    onclick="return confirm('Are you sure?')">@lang('Publish')</button>
                            <button class="btn btn-outline-warning" type="submit" name="status" value="drafted"
                                ...
                                <div class="form-group">
                                    <div class="custom-control custom-switch d-flex ">
                                        <input class="custom-control-input"
                                               ...
                                               type="checkbox"
                                               name="mark_as_clean"
                                               value="1" {{ old('mark_as_clean')?'checked':'' }}>
                                        <label class="custom-control-label"
                                               for="mark_as_clean">@lang('Mark As Clean')
                                        </label>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    <div class="custom-control custom-switch d-flex justify-content-end">
                                        <input
                                            ...
                                            type="checkbox"
                                            name="is_case"
                                            value="1" {{ old('is_case')?'checked':'' }}>
                                        <label
                                            class="custom-control-label"
                                            for="is_case">@lang('Is Case')
                                        </label>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="form-group" v-if="computedSubTypes.length">
                            ...
                            <select name="product_sub_type_id" id="product_sub_type_id" class="custom-select custom-select-lg">
                                <option
                                    v-for="(subType, subTypeIndex) in computedSubTypes"
                                    :key="subTypeIndex"
                                    :value="subType.id"
                                    :selected="subType.id == @json(old('product_sub_type_id'))"
                                >
                                    @{{ subType.name }}
                                </option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="for" id="for" class="custom-select custom-select-lg">
                                <option value="both" {{ old('for') == 'both'?'selected':'' }}>@lang('Both')</option>
                                <option value="him" {{ old('for') == 'him'?'selected':'' }}>@lang('Him')</option>
                                <option value="her" {{ old('for') == 'her'?'selected':'' }}>@lang('Her')</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="stock">@lang('Stock') (-1 for unlimited)</label>
                            <input type="number" step="1" min="-1" class="form-control" name="stock" id="stock" value="{{ old('stock')??-1 }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select2 name="brand_id" id="brand_id" class="form-control" required>
                                <option v-for="(brand, brandIndex) in computedBrands" :key="brandIndex" :value="brand.id" :selected="brand.id == @json(old('brand_id'))">
                                    @{{ brand.name }}
                                </option>
                            </select2>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="notes[]" id="notes" class="form-control selectpicker" multiple>
                                @foreach($notes as $note)
                                    <option value="{{ $note->id }}" {{ is_array(old('notes')) && in_array($note->id, old('notes'))?'selected':'' }}>{{ $note->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="labels[]" id="labels" class="form-control selectpicker" multiple>
                                @foreach($labels as $label)
                                    <option value="{{ $label->id }}" {{ is_array(old('labels')) && in_array($label->id, old('labels'))?'selected':'' }}>{{ $label->label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="families[]" id="families" class="form-control selectpicker" multiple>
                                @foreach($families as $family)
                                    <option value="{{ $family->id }}" {{ is_array(old('families')) && in_array($family->id, old('families'))?'selected':'' }}>{{ $family->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="badges[]" id="badges" class="form-control selectpicker" multiple>
                                @foreach($badges as $badge)
                                    <option value="{{ $badge->id }}" {{ is_array(old('badges')) && in_array($badge->id, old('badges'))?'selected':'' }}>{{ $badge->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="collections[]" id="collections" class="form-control selectpicker" multiple>
                                @foreach($collections as $collection)
                                    <option value="{{ $collection->id }}" {{ is_array(old('collections')) && in_array($collection->id, old('collections'))?'selected':'' }}>{{ $collection->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
    ...
    <script>
        ...
        window.app = new Vue({
            ...
            data: {
                ...
                purchase_options: @json(old('purchase_options')??[]),
                tempPurchaseOption: 'one_time',
                type: @json(old('type')??'perfume'),
                related_products: @json(old('related_products')??[]),
                taxonomies: @json($taxonomies),
                subTypes: @json($subTypes),
                brands: @json($brands),
                terms: @json(old('terms')??[]),
                products: [],
            },
            beforeMount() {
                ...
                if (!this.products.length) {
                    var vm = this
                    $.getJSON(@json(route('staff.select2.data', ['model' => 'product'])), function (data) {
                        vm.products = data
                    })
                }
            },
            methods: {
                ...
                addYoutube() {
                    ...
                    this.images.push({type: 'youtube', thumb_url: 'https://img.youtube.com/vi/' + code + '/hqdefault.jpg', url: 'https://www.youtube.com/embed/' + code})
                }
            },
            computed: {
                giftset_products() {
